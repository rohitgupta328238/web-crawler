...
import utils.{RestService, RestServiceAdaptor}
...
class WebCrawlerService {
  ...
  def processRequest(crawlRequest: CrawlRequest, restService: RestService = RestServiceAdaptor): Future[CrawlResponse] = {
    ...
    val setOfFutureResponse: Set[Future[Either[Throwable, CrawlResponseElement]]] = crawlRequest.urls.map { url =>
      Future{
        Right(CrawlResponseElement(url, restService.restGetCall(url)))
      }.recover {
    ...
    val response: Future[CrawlResponse] = Future.sequence(setOfFutureResponse).map { setOfResults: Set[Either[Throwable, CrawlResponseElement]] =>
      ...
      errors.isEmpty match {
        ...
        case false => // At least one error is there. Concatenate all the errors with (|) and send the response.
